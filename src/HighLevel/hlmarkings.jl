...
HLMarking("the text", Term(:term, IdDict{Symbol, Any}(:value => 3)), )
...
marking_type(::Type{T}) where {T<:AbstractHLCore} = HLMarking{marking_value_type(T)}
marking_value_type(::Type{T}) where {T<:AbstractHLCore} = Term{PnmlDict}

condition_value_type(::Type{<:AbstractHLCore}) = Term{PnmlDict}

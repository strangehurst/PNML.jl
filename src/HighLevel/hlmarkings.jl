...
marking_type(::Type{T}) where {T<:AbstractHLCore} = HLMarking{Term{PnmlDict} }#{marking_value_type(T)}
marking_value_type(::Type{T}) where {T<:AbstractHLCore} = Int

condition_value_type(::Type{<:AbstractHLCore}) = Term{PnmlDict} #! RELOCATE for HL

...
struct Variable{N <: AbstractPnmlNet} <: AbstractVariable
    ...
    function Variable(v::REFID, net::AbstractPnmlNet)
        # Check that REFID is valid in DeclDict.
        PNML.has_variabledecl(decldict(net), v) ||
...
decldict(v::Variable) = decldict(v.net)
...
sortref(v::Variable) = sortref(variabledecl(decldict(v.net), refid(v)))::AbstractSortRef # Access variabledecl in decldicts
sortof(v::Variable)  = sortof(variabledecl(decldict(v.net), refid(v)))  # Access variabledecl in decldicts
